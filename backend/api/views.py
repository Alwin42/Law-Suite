...
class DashboardStatsView(views.APIView):
    ...
    def get(self, request):
        ...
        # 4. Fetch Recent Appointments (Limit 3)
        recent_appointments_qs = Appointment.objects.filter(advocate=user).order_by('-created_at')[:3]
        recent_appointments = [{
            'id': a.id,
            'client_name': a.client.full_name if a.client else "Unknown",
        ...
        # --- NEW: Fetch ALL Dates for the Calendar Widget ---
        calendar_hearings = Case.objects.filter(
            created_by=user, next_hearing__isnull=False
        ).values_list('next_hearing', flat=True)
        ...
        return Response({
            ...
            'upcoming_hearings': upcoming_hearings,
            'recent_appointments': recent_appointments, 
            # Send dates as strings to React
            'calendar_data': {
