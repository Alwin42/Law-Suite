...
const LandingPage = () => {
  ...
  // State to track when the heavy image finishes downloading
  const [isImageLoaded, setIsImageLoaded] = useState(false);

  return (
    <>
      {/* INVISIBLE PRELOADER: Forces the browser to download the heavy image in the background */}
      <img 
        src="/lady-justice.png" 
        style={{ display: "none" }} 
        onLoad={() => setIsImageLoaded(true)} 
        onError={() => setIsImageLoaded(true)} // Failsafe: if image breaks, still let the user in
        alt="Preload lady justice" 
      />
      ...
      <AnimatePresence mode="wait">
        {!isImageLoaded ? (
          ...
          <motion.div 
            ...
            className="relative w-full bg-background overflow-x-hidden"
          >
            ...
            <section className="relative min-h-screen flex items-center pt-20 pb-10 z-10">
              <div className="container mx-auto px-6 grid md:grid-cols-2 gap-12 items-center">
                ...
                {/* Because this section only mounts AFTER the image is loaded, this animation is perfectly synced! */}
                <motion.div 
                  initial={{ opacity: 0, scale: 0.95 }}
                  animate={{ opacity: 1, scale: 1.19 }}
                  transition={{ duration: 0.8, delay: 0.2 }} // Added a slight delay for dramatic effect
                ...
                >
                  <div className="relative h-full w-full flex items-center justify-center">
                    <img 
                        ...
                        // The image is already cached by the preloader, so it renders instantly
