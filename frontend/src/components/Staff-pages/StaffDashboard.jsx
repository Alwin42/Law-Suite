        ...
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white rounded-2xl border border-zinc-200 shadow-sm p-6">
              <h3 className="font-bold text-lg text-zinc-900 mb-6">Action Required</h3>
                ...
                {pendingTasks.length === 0 ? <p className="text-zinc-500 text-sm py-4 text-center">No pending tasks found.</p> : pendingTasks.map(task => (
                  <div key={task.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl border border-zinc-100 hover:border-zinc-300 hover:shadow-sm transition-all bg-zinc-50/50 gap-4">
                    <div className="flex items-start gap-4">
                      <div className={`mt-1 p-2 rounded-lg ${task.type === 'payment' ? 'bg-red-50 text-red-600' : task.type === 'appointment' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
                    ...
                    <button className="w-full sm:w-auto flex items-center justify-center gap-2 px-3 py-2 sm:py-1.5 border border-zinc-200 text-zinc-700 text-xs font-bold rounded-lg hover:bg-zinc-100">
...
const StatCard = ({ title, value, icon: Icon, color }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm flex items-center gap-5">
      <div className={`p-4 rounded-xl border ${colorMap[color]}`}><Icon size={24} /></div>
      <div><p className="text-3xl font-extrabold text-zinc-900">{value}</p><p className="text-xs font-bold text-zinc-500 uppercase tracking-wider mt-1">{title}</p></div>
    </div>
  );
};
...
const QuickActionButton = ({ icon: Icon, label, onClick }) => (
  <button onClick={onClick} className="w-full flex items-center gap-3 p-3 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 transition-colors text-left group">
    <div className="bg-zinc-800 p-2 rounded-lg group-hover:bg-black transition-colors"><Icon size={16} className="text-zinc-300 group-hover:text-white" /></div>
