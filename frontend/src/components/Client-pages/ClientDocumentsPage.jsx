...
import { 
  ...
  AlertCircle, CheckCircle, Menu // <-- Added Menu icon
} from "lucide-react";
    ...
    <div className="flex min-h-screen bg-[#FAFAFA] font-sans text-slate-900 pt-16 overflow-hidden">
      ...
      <aside className={`w-72 bg-white/95 backdrop-blur-xl border-r border-slate-100 flex flex-col fixed left-0 top-16 bottom-0 z-50 shadow-2xl lg:shadow-none transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto mt-2 lg:mt-6">
          <SidebarItem icon={LayoutDashboard} label="Overview" onClick={() => handleNavClick("/client-dashboard")} />
          <SidebarItem icon={Briefcase} label="My Cases" onClick={() => handleNavClick("/client-dashboard/cases")} />
          <SidebarItem icon={Gavel} label="Hearings" onClick={() => handleNavClick("/client-dashboard/hearings")} />
          <SidebarItem icon={CreditCard} label="Billing & Payments" onClick={() => handleNavClick("/client-dashboard/payments")} />
      ...
      <main className={`flex-1 p-4 md:p-8 lg:p-12 overflow-y-auto h-[calc(100vh-4rem)] transition-all duration-300 ease-in-out ${isSidebarOpen ? 'lg:ml-72' : 'ml-0'}`}>
        <div className="max-w-6xl mx-auto">
            ...
            {/* HEADER */}
            <motion.header initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 md:gap-6 mb-8 md:mb-10">
              <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight">Case Documents</h1>
                    <p className="text-sm md:text-base text-slate-500 mt-1 md:mt-2">Securely view and upload files related to your legal matters.</p>
                  </div>
              </div>
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                <DialogTrigger asChild>
                  <Button className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white shadow-lg shrink-0">
                    <UploadCloud className="mr-2 h-4 w-4" /> Upload Document
                  </Button>
                </DialogTrigger>
                <DialogContent className="bg-white max-w-[90%] sm:max-w-[425px] rounded-2xl md:rounded-xl">
                  <DialogHeader>
                    <DialogTitle>Upload New Document</DialogTitle>
                  </DialogHeader>
                  <form onSubmit={handleUpload} className="space-y-4 mt-2 md:mt-4">
                    
                    <div className="space-y-2">
                      <Label>Select Case</Label>
                      <Select name="case" required>
                        <SelectTrigger className="bg-slate-50 border-slate-200 h-10 md:h-11">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input name="title" placeholder="Ex: ID Proof, Evidence Photo..." required className="bg-slate-50 border-slate-200 h-10 md:h-11"/>
                    </div>

                    <div className="space-y-2">
                      <Label>File Attachment</Label>
                      <Input name="document" type="file" required className="bg-slate-50 border-slate-200 cursor-pointer h-10 md:h-11 py-2 md:py-2.5"/>
                    </div>

                    <Button type="submit" disabled={isUploading} className="w-full h-10 md:h-11 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {documents.length > 0 ? documents.map((doc) => (
                <motion.div 
                  ...
                  key={doc.id} 
                  className="group bg-white p-4 md:p-5 rounded-2xl border border-slate-200 hover:border-emerald-200 hover:shadow-lg transition-all flex flex-col justify-between"
                >
                  <div className="flex items-start gap-4">
                    <div className="p-2.5 md:p-3 bg-slate-50 rounded-xl group-hover:bg-emerald-50 transition-colors shrink-0">
                      <FileText size={20} className="text-slate-400 group-hover:text-emerald-600 md:w-6 md:h-6"/>
                    </div>
                    <div className="overflow-hidden">
                      <h3 className="font-bold text-slate-900 line-clamp-1 text-sm md:text-base" title={doc.title}>{doc.title}</h3>
                      <p className="text-[10px] md:text-xs text-slate-500 mt-1">
                        {new Date(doc.uploaded_at).toLocaleDateString()}
                      </p>
                      <span className="inline-block mt-2 px-2 py-0.5 bg-slate-100 text-slate-600 text-[9px] md:text-[10px] font-bold uppercase tracking-wider rounded-md truncate max-w-full">
                        CASE #{doc.case}
                      </span>
                    </div>
                  </div>

                  <div className="mt-5 md:mt-6 pt-4 border-t border-slate-50 flex justify-between items-center">
                    ...
                    <a 
                      ...
                      title="Download / View"
                    >
                      <Download size={18} className="w-4 h-4 md:w-[18px] md:h-[18px]" />
                    </a>
                  </div>
                </motion.div>
              )) : (
                <div className="col-span-full py-16 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50 px-4 text-center">
                  <UploadCloud size={40} className="text-slate-300 mb-4 md:w-12 md:h-12" />
                  <p className="text-slate-500 font-medium text-sm md:text-base">No documents uploaded yet.</p>
                </div>
              )}
            </div>
        </div>
